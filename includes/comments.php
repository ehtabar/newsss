...
function wp_review_comment_count( $count ) {
	if ( ! is_admin() ) {
		...
		if ( isset( $wp_query->comments_by_type['comment'] ) ) {
			$wp_review_comments_count = isset( $wp_query->comments_by_type['wp_review_comment'] ) ? count( $wp_query->comments_by_type['wp_review_comment'] ) : 0;
			return count( $wp_query->comments_by_type['comment'] ) + $wp_review_comments_count;
		} else {
			return $count;
		}
	}
...
function wp_review_preprocess_comment( $commentdata ) {
    ...
    } elseif ( ! empty( $options['require_rating'] ) && $review_through_comment && ! hasPreviousReview( $commentdata['comment_post_ID'], $user_id, $ip, WP_REVIEW_COMMENT_TYPE_COMMENT ) ) {
